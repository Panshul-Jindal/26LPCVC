...
DLC_PATH  = "./export_assets/resnet_2plus1d.bin"  # e.g. "./export_assets/resnet_2plus1d.bin"
...
# Maximum number of samples to use for inference.
# Set to None to use ALL samples (may crash laptop on 26GB dataset!).
# 200 = ~400MB upload, representative across all 92 classes (~2 per class).
# 92  = exactly 1 per class, minimal upload, proves full label coverage.
MAX_SAMPLES = 200

# Output HDF5 file written after inference, consumed by evaluate.py.
OUTPUT_H5 = "dataset-export.h5"
# Subset manifest written for the sampled files, so evaluate.py has
# the correct ground-truth labels in the right order.
SAMPLED_MANIFEST = "sampled_manifest.jsonl"
...
def load_video_tensors(root: str, max_samples: int | None = None) -> tuple[list[np.ndarray], list[str]]:
    """Load tensors with optional stratified sampling across class folders.
    Returns (tensors, selected_paths) so callers can write a subset manifest.
    """
    import random
    random.seed(42)  # reproducible results

    # Collect all paths grouped by class
    class_paths: dict[str, list[str]] = {}
    for cls in sorted(os.listdir(root)):
        cls_dir = os.path.join(root, cls)
        if not os.path.isdir(cls_dir):
            continue
        paths = sorted([os.path.join(cls_dir, f) for f in os.listdir(cls_dir) if f.endswith(".npy")])
        if paths:
            class_paths[cls] = paths

    if max_samples is None:
        # Load everything
        selected = [p for paths in class_paths.values() for p in paths]
        selected = sorted(selected)
    else:
        # Stratified: pick evenly from each class
        selected = []
        classes = sorted(class_paths.keys())
        per_class = max(1, max_samples // len(classes))
        for cls in classes:
            chosen = class_paths[cls][:per_class]  # take first N (already sorted)
            selected.extend(chosen)
            if len(selected) >= max_samples:
                break
        selected = selected[:max_samples]
        print(f"Stratified sample: {len(selected)} tensors from {len(classes)} classes (~{len(selected)//len(classes)} per class)")
...
if USE_SINGLE_TENSOR:
    ...
    video_tensors, selected_paths = load_video_tensors(data_path, max_samples=MAX_SAMPLES)

print(f"Loaded {len(video_tensors)} samples (MAX_SAMPLES={MAX_SAMPLES})")

# Write a subset manifest so evaluate.py has labels in the correct order
if selected_paths and SAMPLED_MANIFEST:
    import json as _json
    path_to_label: dict[str, str] = {}
    full_manifest = os.path.join(data_path, "manifest.jsonl")
    if os.path.exists(full_manifest):
        with open(full_manifest, "r") as _mf:
            for _line in _mf:
                _rec = _json.loads(_line)
                path_to_label[_rec["tensor_path"]] = _rec["label"]
    with open(SAMPLED_MANIFEST, "w") as _sf:
        for _path in selected_paths:
            _label = path_to_label.get(_path, os.path.basename(os.path.dirname(_path)))
            _sf.write(_json.dumps({"tensor_path": _path, "label": _label}) + "\n")
    print(f"Wrote subset manifest: {SAMPLED_MANIFEST} ({len(selected_paths)} entries)")

...
def download_model(
    ...
    try:
        ...
        # Fallback: if metadata schema changed between versions, do a simple direct download
        print(f"\n[Warning] Metadata packaging failed ({e})")
        print(f"[Fallback] Saving .bin directly to {output_path}/")
        os.makedirs(output_path, exist_ok=True)
        bin_path = os.path.join(output_path, f"{model_name}.bin")
        target_model.download(bin_path)
